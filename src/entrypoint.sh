...
invoke () {
    ...
    then
        ...
        /usr/local/bin/invoke $@ >> /usr/src/geo_dprkp/invoke.log 2>&1
    fi
    echo "$@ tasks done"
}
...
then
    ...
    then
        ...
        if [ ${FORCE_REINIT} = "true" ]  || [ ${FORCE_REINIT} = "True" ] || [ ! -e "/mnt/volumes/statics/geonode_init.lock" ]; then
            echo "LOG INIT" > /usr/src/geo_dprkp/invoke.log
            invoke updategeoip
            invoke fixtures
...
cp  /usr/src/geo_dprkp/uwsgi.ini.0  /usr/src/geo_dprkp/uwsgi.ini
